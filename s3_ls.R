...
s3_ls <- function(s3_folder, config=NULL, recursive=FALSE) {
    require(stringr)
    if (!is.null(config)) {
        # Needed on Windows
        Sys.setenv(AWS_CONFIG_FILE=config)
    }
    ls_args <- c('s3', 'ls', s3_folder)
    if (recursive) ls_args <- c(ls_args, '--recursive')
    s3_ls <- system2('aws', args=ls_args, stdout=TRUE)
    s3_files <- gsub(' [ ]*', ' ', s3_ls)
